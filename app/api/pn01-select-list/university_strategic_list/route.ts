...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const insertQuery = `
      INSERT INTO university_strategic_list (id, name)
      VALUES ($1, $2)
    `;

    const deleteQuery = `
      DELETE FROM university_strategic_list WHERE id = $1
    `;

    const existingRows = await pool.query(
      'SELECT * FROM university_strategic_list',
    );
    ...
    for (const { id, name } of formData) {
      const existingRow = await pool.query(
        'SELECT * FROM university_strategic_list WHERE id = $1',
        [id],
      );
      if (existingRow.rows.length === 0) {
        ...
        await pool.query(
          'UPDATE university_strategic_list SET name = $1 WHERE id = $2',
          [name, id],
        );
      }
    }
