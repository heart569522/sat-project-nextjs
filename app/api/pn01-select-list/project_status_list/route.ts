...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const insertQuery = `
      INSERT INTO project_status_list (id, name)
      VALUES ($1, $2)
    `;

    const deleteQuery = `
      DELETE FROM project_status_list WHERE id = $1
    `;

    const existingRows = await pool.query('SELECT * FROM project_status_list');
    ...
    for (const { id, name } of formData) {
      const existingRow = await pool.query(
        'SELECT * FROM project_status_list WHERE id = $1',
        [id],
      );
      if (existingRow.rows.length === 0) {
        ...
        await pool.query(
          'UPDATE project_status_list SET name = $1 WHERE id = $2',
          [name, id],
        );
      }
    }
